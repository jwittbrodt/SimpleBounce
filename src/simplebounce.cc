...
// phivec(i)
double *Scalarfield::phivec(const int i) const { return &phi_[i * nphi_ + 0]; }

// radius : r_i = i * dr
double Scalarfield::r(const int i) const { return dr_ * i; }
...
int Scalarfield::n() const { return n_; }

// return the number of the scalar field(s)
int Scalarfield::nphi() const { return nphi_; }

// return the dimension of space
int Scalarfield::dim() const { return dim_; }

// return the radius at the boundary
double Scalarfield::rmax() const { return rmax_; }

// return the lattice spacing
double Scalarfield::dr() const { return dr_; }
...
/*
 * calculate the bounce action by a gradient flow equation which is proposed by
 * 1907.02417. The gradient flow equation solves Coleman-Glaser-Martin's reduced
 * problem. See, https://doi.org/10.1007/BF01609421 The bounce solution can be
 * obtained from scale transformation. See rBounce() and action().
 */
...
// set scalar potential and its derivatives to be used for the bounce
// calculation
...
double BounceCalculator::t() const {
    ...
    for (int i = 0; i < n() - 1; i++) {
        ...
        for (int iphi = 0; iphi < nphi(); iphi++) {
            integrand[i] +=
                r_dminusoneth(i) * -0.5 * phi(i, iphi) * lap(i, iphi);
        }
    }
...
double BounceCalculator::evolve(const double dtau) {
    ...
    // \nabla^2\phi_i integral2 : \int_0^\infty dr r^{d-1} \sum_i (\partial V /
    // \partial\phi_i)^2
...
// The bounce configuration can be obtained from Eq. 15 of 1907.02417
double BounceCalculator::action() const { return tBounce() + vBounce(); }
...
int BounceCalculator::setVacuum(const double *phiTV_, const double *phiFV_) {
    ...
    if (model->vpot(phiTV_) > model->vpot(phiFV_)) {
        std::cerr << "!!! energy of true vacuum is larger than false vacuum !!!"
                  << std::endl;
        return -1;
    }
...
int BounceCalculator::evolveUntil(const double tauend) {
    ...
    // 1 + d + sqrt(1 + d) is maximum of absolute value of eigenvalue of {{-2d,
    // 2d},{ (1-d)/2 + 1, -2}}, which is discreitzed Laplacian for n = 2. This
    // value is 6 for d=3, and 7.23607 for d=4. The numerical value of maximum
    // of absolute value of eigenvalue of discretized Laplacian for large n is 6
    // for d=3, and 7.21417 for d=4
    double dtau =
...
int BounceCalculator::solve() {
    ...
    // if dphi/dr at the boundary becomes too large during flow, take smaller
...
double BounceCalculator::getlambda() const { return lambda; }
...
void BounceCalculator::setSafetyfactor(double x) { safetyfactor = x; }

// set the parameter to determine the maximal variation of the field in evolve()
void BounceCalculator::setMaximumvariation(double x) { maximumvariation = x; }

// set the initial value of the parameter to determine the initail configuration
void BounceCalculator::setXTV0(double x) { xTV0 = x; }

// set the initial value of the parameter to determine the initail configuration
void BounceCalculator::setWidth0(double x) { width0 = x; }

// set the maximal value of the derivative of field at the boundary
void BounceCalculator::setDerivMax(double x) { derivMax = x; }

// set tau0
void BounceCalculator::setTend0(double x) { tend0 = x; }

// set tau1
void BounceCalculator::setTend1(double x) { tend1 = x; }

// set the maximal value of the grid
void BounceCalculator::setMaxN(int x) { maxN = x; }

// turn on verbose mode
void BounceCalculator::verboseOn() { verbose = true; }

// turn off verbose mode
void BounceCalculator::verboseOff() { verbose = false; }
